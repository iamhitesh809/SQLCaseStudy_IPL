...
--Q2 Find the top3 highest paid 'All rounders' across all teams
select Top 3 * from IPLPlayers where Role='All-rounder' order by Price_in_cr Desc;
--Q3 Find the highest-priced player in each team
    ...
    FROM (
        SELECT 
            ...
            Price_in_cr,
            Team,
            rank() OVER (PARTITION BY Team ORDER BY Price_in_cr DESC) AS DR
        FROM IPLPlayers
    ) t
    WHERE DR = 1;
